...
class MidTmcLeafNodeDish(TMCBaseLeafDevice):
    ...
    def StartAvailabilityFlood(self, total: int) -> None:
        ...
        device's request-serving thread is NOT occupied. On this (fixed) build
        the change-guard makes all but the first call no-ops, so the bus stays
        quiet and a concurrent client read should remain fast.
        """
        import threading
